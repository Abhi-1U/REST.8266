...
def request_handler(conn,request):
    ...
    if request.find('/?freq=PS') == 6:
        ...
        response(conn,'frequency',freq())
        return True
    if request.find('/device') == 6:
       keys=('Board','Arch','Firmware','FW-version','RAM','External-ROM')
       data=('ESP8266EX','Xtensa 32bit','MicroPython','v1.13','96KB',str(esp.flash_size()))
       response_loop(conn,'device',keys,data)
       return True
    if request.find('/rtc') == 6:
        keys=('Year','Month','Day','WeekDay-Number','Hour','Minute','Second','MilliSeconds')
        data=rtc.datetime()
        response_loop(conn,'rtc',keys,data)
        return True
    if request.find('/adc') ==6:
        adc = ADC(0)
        response(conn,'adc-level',adc.read())
        return True
    if request.find('/lsdir')== 6:
        response(conn,'dir',os.listdir())
        return True
    if request.find('/?open=')==6:
        fname=request[request.find('=')+1:request.find('HTTP')-1]
        response_read(conn,fname)
        return True
    else:
        conn.close()
        return False
def response_read(conn,filename):
    conn.send('HTTP/1.1 200 OK\n')
    conn.send('Server: REST.8266\n')
    conn.send('Content-Type: text/document\n')
